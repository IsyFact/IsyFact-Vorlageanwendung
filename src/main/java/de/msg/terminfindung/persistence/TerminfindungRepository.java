...
public interface TerminfindungRepository extends CrudRepository<Terminfindung, Long> {
     ...
     */
    @Query("select tf from Terminfindung tf join tf.termine as t where tf.defZeitraum in elements(t.zeitraeume) and t.datum <= :datum")
     ...
     */
    Terminfindung findByIdRef(String uuid);
